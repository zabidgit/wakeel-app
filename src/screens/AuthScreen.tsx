...
export function AuthScreen({ navigation }: Props) {
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      // Compose full name (only available on first sign-in)
      const fullName = credential.fullName
        ? [credential.fullName.givenName, credential.fullName.familyName].filter(Boolean).join(' ')
        : null;

      const result = await signInWithAppleOnServer(
        ...
        <View style={styles.buttonSection}>
          ...
          {appleAvailable ? (
            <AppleAuthentication.AppleAuthenticationButton
              ...
              cornerRadius={14}
              style={[styles.appleButton, isLoading && styles.buttonLoading]}
              onPress={handleAppleSignIn}
            />
          ) : (
            <TouchableOpacity
              style={[styles.appleButtonFallback, isLoading && styles.buttonLoading]}
              onPress={handleAppleSignIn}
              activeOpacity={0.85}
              disabled={isLoading}
            >
...
const styles = StyleSheet.create({
  ...
  loadingOverlaySmall: {
    position: 'absolute',
    right: spacing.md,
    bottom: spacing.md + 52 + spacing.sm + 6,
  },
  divider: {
    flexDirection: 'row',
    alignItems: 'center',
